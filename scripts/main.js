...
function searchTrack(track_name, artist) {
	console.log(track_name,artist)

  var search_URI = "https://api.musixmatch.com/ws/1.1/track.search?q_track=" + track_name + "&q_artist=" + artist + "&apikey=d8951a826384c648324e206c942b5cce&format=jsonp&callback=?"
  search_URI = encodeURI(search_URI)
  ...
  return new Promise(function(resolve, reject) {
      getJSON(search_URI).then(function(response) {
        ...
        if (response.message.header.status_code == 200 && response.message["body"].length != 0) {
          ...
          tr = response.message["body"]["track_list"][0]
...
function getLyrics(track) {
  ...
  var lyrics_URI = mxmAPI_base + 'track.subtitles.get?apikey=d8951a826384c648324e206c942b5cce&track_id=' + track.track.track_id + '&subtitle_format=mxm' + '&format=jsonp&callback=?'
...
function drawChart(trackWithLyrics,root) {
  // console.log(lyricsPhrases)
	lyricsPhrases = trackWithLyrics['subtitles']
  ...
  for (var i = 0; i < lyricsPhrases.length; i++) {
    ...
    if (lyricsPhrases[i] != "") {
      tempArr = new Array()
      tempArr.push(lyricsPhrases[i]['text'])
...
$("#make_tree").click(function(event) {
  // if (event.keyCode == 13) {
    // myStopFunction();
    ...
    searchTrack(song,artist).then(function(track) {
        return getLyrics(track)
      }).then(function(phrases) {
        // console.log(discographySentiments)
				root = document.getElementById('root_word').value
        google.visualization.events.addListener(chart, 'ready', readyHandler)
        drawChart(phrases, root)
